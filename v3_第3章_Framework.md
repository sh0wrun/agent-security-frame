...
- **账号 / 设备画像**:关联图谱 + 风险联动。
- **模型上下文运营**(本书在 R1 四维基础上扩展的第五维):记忆完整性 + 系统提示词漂移 + 知识库更新审计。

前四维直接源自 R1 奇安信内部运营实践;第五维是本书对 R9 CIK 中 Knowledge 维度被运营层长期忽视的现实的工程化回应。R9 实证表明,单是对 Knowledge 维度投毒就能使攻击成功率从 24.6% 跃升至 64-74%——若运营画像只覆盖前四维,Knowledge 维度的风险就在视野之外被放大。
...
安全机制应当对正常业务**尽量无感**。亚毫秒级决策(立论 1.4)让用户无法感知策略检查的存在;绿色操作自动放行(立论 1.3)让日常工作流零中断;审批流只对红色操作触发。本约束继承自 R2 SlowMist 的"Zero-friction daily operations"实践——本书在中文化时采用"低摩擦"而非"零摩擦":审批流程永远存在不可消除的最小摩擦,用"低"取代"零"是为了准确表达工程现实,而非削弱原意。R2 给出的具体方法学:用 90% 的自动化放行换 10% 的强制审批,资源集中在真正高危的操作上。
...
- **R1 奇安信 OpenClaw 安全使用指南**(5 维画像运营前 4 维的来源)
- **R2 SlowMist OpenClaw 安全实践指南**(低摩擦运营 + 夜间审计源)
